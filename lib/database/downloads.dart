...
Future<List<int>> getDownloadsCount({Database db}) async {
  ...
  try {
    int total =  Sqflite.firstIntValue(await db.rawQuery('SELECT COUNT(*) from $messageTable WHERE isdownloaded = 1'));
    int played =  Sqflite.firstIntValue(await db.rawQuery('SELECT COUNT(*) from $messageTable WHERE isdownloaded = 1 AND isplayed = 1'));
    return [total, played];
  } catch (error) {
